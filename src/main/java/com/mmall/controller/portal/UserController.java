...
public class UserController {
     ...
     * @return
     */
    @RequestMapping(value = "/logput.do", method = RequestMethod.POST)
    @ResponseBody
    ...
    public ServerResponse<User> getUserInfo(HttpSession session) {
        ...
        return ServerResponse.createByErrorByMessage("用户未登录，无法获得用户信息");
    }
    ...
    public ServerResponse<User> getInformation(HttpSession session){
        User currentUser = (User)session.getAttribute(Const.CURRENT_USER);
        if(currentUser == null){
            return ServerResponse.createByErrorCodeByMessage(ResponseCode.NEED_LOGIN.getCode(),"未登录,需要强制登录status=10");
        }
